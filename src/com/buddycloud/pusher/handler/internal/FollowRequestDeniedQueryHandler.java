 ...
 *
 */
public class FollowRequestDeniedQueryHandler extends AbstractQueryHandler {

	public static final String NAMESPACE = "http://buddycloud.com/pusher/followrequestdenied";
	
	/**
	 * @param namespace
	 * @param properties
	 */
	public FollowRequestDeniedQueryHandler(Properties properties, DataSource dataSource) {
	...
	protected IQ handleQuery(IQ iq) {
		...
		for (NotificationSettings notificationSettings : allNotificationSettings) {
			...
			pusher.push(notificationSettings.getTarget(), Event.FOLLOW_REQUEST_DENIED, tokens);
		}
		
		return createResponse(iq, "User [" + followerJid + "] had its request to " +
				"follow channel [" + channelJid + "] denied.");
	}
}
